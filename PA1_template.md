...
```r
hist(totalSteps$steps,breaks=50)
```
...
hist(totalSteps2$steps,breaks=50)
```
...
* Create a new factor variable in the dataset with two levels ?? ?eekday?? and ?eekend?? indicating whether a given date is a weekday or weekend day.
